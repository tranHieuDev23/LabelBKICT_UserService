...
export class ApplicationConfig {
    ...
    public static fromEnv(): ApplicationConfig {
        ...
        if (process.env.USER_SERVICE_FIRST_USER_DISPLAY_NAME !== undefined) {
            config.firstUserDisplayName =
                process.env.USER_SERVICE_FIRST_USER_DISPLAY_NAME;
        }
        if (process.env.USER_SERVICE_FIRST_USER_USERNAME !== undefined) {
            config.firstUserUsername =
                process.env.USER_SERVICE_FIRST_USER_USERNAME;
        }
        if (process.env.USER_SERVICE_FIRST_USER_PASSWORD !== undefined) {
            config.firstUserPassword =
                process.env.USER_SERVICE_FIRST_USER_PASSWORD;
        }
        if (
            process.env.USER_SERVICE_FIRST_USER_ROLE_DISPLAY_NAME !== undefined
        ) {
            config.firstUserRoleDisplayName =
                process.env.USER_SERVICE_FIRST_USER_ROLE_DISPLAY_NAME;
        }
        if (
            process.env.USER_SERVICE_FIRST_USER_ROLE_DESCRIPTION !== undefined
        ) {
            config.firstUserRoleDescription =
                process.env.USER_SERVICE_FIRST_USER_ROLE_DESCRIPTION;
        }
        if (
