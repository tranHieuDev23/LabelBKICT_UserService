...
export interface UserHasUserTagDataAccessor {
    ...
    getUserListOfUserTag(userTagId: number): Promise<User[]>;
...
export class UserHasUserTagDataAccessorImpl implements UserHasUserTagDataAccessor {
    ...
    public async getUserTagListOfUser(userId: number): Promise<UserTag[]> {
        try {
            const rows = await this.knex
                ...
                .orderBy(`${TabNameUserServiceUserHasUserTag}.${ColNameUserServiceUserHasUserTagUserId}`, "asc");
    ...
    public async getUserTagIdListOfUser(userId: number): Promise<number[]> {
        try {
            const rows = await this.knex
                ...
                .orderBy(ColNameUserServiceUserHasUserTagUserTagId, "asc");
    ...
    public async getUserIdListOfUserTagList(userTagIdList: number[]): Promise<number[]> {
        try {
            ...
            this.logger.error("failed to get user id list of user tag id list", { error });
            throw ErrorWithStatus.wrapWithStatus(error, status.INTERNAL);
        }
    }

    public async getUserListOfUserTag(userTagId: number): Promise<User[]> {
        try {
            const rows = await this.knex
                .select()
                .from(TabNameUserServiceUserHasUserTag)
                .join(
                    TabNameUserServiceUser,
            ...
            this.logger.error("failed to get user list of user tag id list", { error });
            throw ErrorWithStatus.wrapWithStatus(error, status.INTERNAL);
        }
    }
