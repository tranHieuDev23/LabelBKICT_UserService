...
export interface UserTagDataAccessor {
    ...
    withTransaction<T>(
        executeFunc: (dataAccessor: UserTagDataAccessor) => Promise<T>
    ): Promise<T>;
}
...
export class UserTagDataAccessorImpl implements UserTagDataAccessor {
            ...
            await this.knex
                ...
                .update({
                    [ColNameUserServiceUserTagDisplayName]:
                        userTag.displayName,
                    [ColNameUserServiceUserTagDescription]:
                        userTag.description,
                })
                .where({
    ...
    public async deleteUserTag(id: number): Promise<void> {
        ...
        if (deletedCount === 0) {
            ...
            throw new ErrorWithStatus(
                `no user tag with user_tag_id ${id} found`,
                status.NOT_FOUND
            );
        }
    }
    ...
    private async getUserListPaginationKeyset(
        ...
        try {
            ...
            this.logger.error(
                "failed to get user tag list pagination keyset",
                { error }
            );
            throw ErrorWithStatus.wrapWithStatus(error, status.INTERNAL);
        }
            ...
            case UserTagListSortOrder.ID_ASCENDING:
                queryBuilder = this.knex
                    ...
                    .where(
                        ColNameUserServiceUserTagId,
                        ">=",
                        keyset[ColNameUserServiceUserTagId]
                    )
                    .orderBy(ColNameUserServiceUserTagId, "asc");
                break;
    ...
    public async getUserTag(id: number): Promise<UserTag | null> {
        ...
        if (rows.length > 1) {
            this.logger.error(
                "more than one user tag with user_tag_id found",
                { userTagId: id }
            );
            throw new ErrorWithStatus(
                "more than one user tag was found",
                status.INTERNAL
            );
        }
    ...
    public async withTransaction<T>(
        ...
        return this.knex.transaction(async (tx) => {
            const txDataAccessor = new UserTagDataAccessorImpl(
                tx,
                this.logger
            );
...
export const USER_TAG_DATA_ACCESSOR_TOKEN = token<UserTagDataAccessor>(
    "UserTagDataAccessor"
);
