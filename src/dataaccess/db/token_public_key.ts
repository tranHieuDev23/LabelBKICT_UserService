    ...
    public async createTokenPublicKey(data: string): Promise<number> {
        try {
            const rows = await this.knex
                .insert({
                    [ColNameUserServiceTokenPublicKeyData]: data,
                })
    ...
    public async getTokenPublicKey(id: number): Promise<TokenPublicKey | null> {
        ...
        try {
            rows = await this.knex
                ...
                .where({
                    [ColNameUserServiceTokenPublicKeyId]: id,
                });
        } catch (error) {
...
export const TOKEN_PUBLIC_KEY_DATA_ACCESSOR_TOKEN = token<TokenPublicKeyDataAccessor>("TokenPublicKey");
