...
export class TokenManagementOperatorImpl implements TokenManagementOperator {
    ...
    constructor(
        ...
        this.renewTimeInMs = ms(tokenConfig.jwtRenewTime);
    }
    ...
    public async getUserFromToken(token: string): Promise<{ user: User | null; newToken: string | null }> {
        ...
        if (user === null) {
            ...
            throw new ErrorWithStatus(`no user with user_id ${decodedResult.userId} found`, status.UNAUTHENTICATED);
        }
    ...
    public async blacklistToken(token: string): Promise<void> {
        ...
        return this.blacklistedTokenDM.withTransaction(async (dm) => {
            const expireAt = await dm.getBlacklistedTokenExpireAtWithXLock(decodedResult.tokenId);
            if (expireAt !== null) {
                this.logger.info("token is blacklisted", {
                    tokenId: decodedResult.tokenId,
                });
            ...
            await dm.createBlacklistedToken(decodedResult.tokenId, decodedResult.expireAt);
        });
    }
    ...
    private async isTokenBlacklisted(tokenId: number): Promise<boolean> {
        const expireAtTime = await this.blacklistedTokenDM.getBlacklistedTokenExpireAt(tokenId);
