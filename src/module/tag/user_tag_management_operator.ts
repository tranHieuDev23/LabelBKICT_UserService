...
export class UserTagManagementOperatorImpl implements UserTagManagementOperator {
    constructor(
    ...
    public async createUserTag(displayName: string, description: string): Promise<UserTag> {
        ...
        if (!this.isValidUserTagDisplayName(displayName)) {
            ...
            throw new ErrorWithStatus("invalid user tag display name", status.INVALID_ARGUMENT);
        }
        ...
        if (!this.isValidUserTagDescription(description)) {
            ...
            throw new ErrorWithStatus("invalid user tag description", status.INVALID_ARGUMENT);
        }

        const userTagId = await this.userTagDM.createUserTag(displayName, description);
        return {
            id: userTagId,
    ...
    public async updateUserTag(userTag: UserTag): Promise<UserTag> {
        if (userTag.id === undefined) {
            ...
            throw new ErrorWithStatus("user_tag.id is required", status.INVALID_ARGUMENT);
        }
        const userTagId = userTag.id;

        if (userTag.displayName !== undefined) {
            userTag.displayName = this.sanitizeUserTagDisplayName(userTag.displayName);
        ...
        if (userTag.description !== undefined) {
            userTag.description = this.sanitizeUserTagDescription(userTag.description);
        ...
        return this.userTagDM.withTransaction(async (dm) => {
            ...
            if (userTagRecord === null) {
                ...
                throw new ErrorWithStatus(`no user tag with user_tag_id ${userTagId}`, status.NOT_FOUND);
            }
    ...
    public async addUserTagToUser(userId: number, userTagId: number): Promise<void> {
        ...
        if (userTagRecord === null) {
            ...
            throw new ErrorWithStatus(`no user tag with user_tag_id ${userTagId} found`, status.NOT_FOUND);
        }

        return this.userHasUserTagDM.withTransaction(async (dm) => {
            const userHasUserTag = await dm.getUserHasUserTagWithXLock(userId, userTagId);
    ...
    public async getUserTagListOfUser(userId: number): Promise<UserTag[]> {
        return await this.userHasUserTagDM.getUserTagListOfUser(userId);
    }
    ...
    public async getUserListOfUserTagByDisplayName(displayName: string): Promise<User[]> {
        ...
        const userTagRecord = await this.userTagDM.getUserTagByDisplayName(displayName);
        if (userTagRecord === null) {
            this.logger.error("no user tag with display name found", { displayName });
            throw new ErrorWithStatus(`no user tag with display name ${displayName} found`, status.NOT_FOUND);
        }
        return await this.userHasUserTagDM.getUserListOfUserTag(userTagRecord.id);
    }
    ...
    private getUserTagListSortOrder(sortOrder: _UserTagListSortOrder_Values): UserTagListSortOrder {
        switch (sortOrder) {
...
export const USER_TAG_MANAGEMENT_OPERATOR_TOKEN = token<UserTagManagementOperator>("UserTagManagementOperator");
